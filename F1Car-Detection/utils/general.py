...
def check_dataset(data, autodownload=True):
    ...
    if isinstance(data, (str, Path)) and str(data).endswith('.zip'):
        download(data, dir='../datasets', unzip=True,
    ...
    train, val, test, s = [data.get(x) for x in (
        'train', 'val', 'test', 'download')]
    if val:
        val = [Path(x).resolve()
...
def print_mutation(results, hyp, save_dir, bucket):
    evolve_csv, results_csv, evolve_yaml = save_dir / \
        'evolve.csv', save_dir / 'results.csv', save_dir / 'hyp_evolve.yaml'
